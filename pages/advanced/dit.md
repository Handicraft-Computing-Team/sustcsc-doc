...
Diffusion扩散模型不仅仅在文生图任务中应用广泛，在目前工业大模型中，以Transformer作为骨架网络的Diffusion扩散模型也得到广泛的应用，比如在蛋白质结构预测的AlphaFold3、在VLM（Vision-Language-Model）任务中的pi0模型都采用了类似结构，对Diffusion模型的训推加速仍然是目前学术界和工业界的热门话题。
...
5. **评测**
   ...
   - 性能分：性能分得分请参考以下标准
     - Baseline时间为500s（平均单图生成速度，即总时间除以1000得到的推理时间）
     - 评分公式：
      $$
      \text{Score}(T) = \left\{
      \begin{aligned}
