...
**7月27日新增：如果有队伍在WRF I/O上取得了一定的优化效果并且在报告中体现（例如绘制图表展示并说明优化流程），将获得10分加分（即使没能在1小时内跑完6小时的模拟量）；对于WRF I/O优化不一定需要用Bonus 6小时的算例进行测试，也可以使用之前conus 2.5 km数据集10分钟模拟的算例**。
